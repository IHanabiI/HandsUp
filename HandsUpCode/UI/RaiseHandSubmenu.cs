...
public partial class RaiseHandSubmenu : NSubmenu
{
    ...
    private const string RestartLabel = "\u91cd\u5f00";
    private const string SoftRestartLabel = "\u5c40\u90e8\u91cd\u5f00";
    private const string PreviousStepLabel = "\u56de\u9000\u5230\u4e0a\u4e00\u6b65";
    private const string PreviousFloorLabel = "\u56de\u9000\u5230\u4e0a\u4e00\u5c42";
    ...
    private const string ConfirmBodyText =
        "[center]\u4f60\u60f3\u597d\u4e86\u5417\u9a9a\u5e74[/center]\n" +
        "[center][font_size=28][color=#d25a5a]\u56de\u9000\u7684\u8fdb\u5ea6\u5c06\u6c38\u8fdc\u6d88\u5931[/color][/font_size][/center]";
    private const string PreviousStepConfirmBodyText =
        "[center]\u4f60\u60f3\u597d\u4e86\u5417\u9a9a\u5e74[/center]\n" +
        "[center][font_size=28][color=#d25a5a]\u56de\u9000\u7684\u8fdb\u5ea6\u5c06\u6c38\u8fdc\u6d88\u5931[/color][/font_size][/center]\n" +
        "[center]\u6b64\u9b54\u6cd5\u4e0d\u7a33\u5b9a\uff01\uff01\uff01[/center]";
    ...
    private void BuildButtons()
    {
        ...
        foreach (var label in ActionLabels)
        {
            ...
            button.Connect(NClickableControl.SignalName.Released, Callable.From<NButton>(_ => OnActionPressed(label)));
    ...
    private void OnActionPressed(string actionLabel)
    {
        ShowConfirmPopup(actionLabel);
    }
    ...
    private void ConfirmAction(string actionLabel)
    {
        ...
        {
            SetStatusText("\u5df2\u786e\u8ba4\u5c40\u90e8\u91cd\u5f00\uff0c\u6b63\u5728\u8bfb\u53d6\u6700\u8fd1\u7684\u5b58\u6863\u8282\u70b9\u3002");
        ...
        {
            SetStatusText("\u5df2\u786e\u8ba4\u56de\u9000\u5230\u4e0a\u4e00\u5c42\uff0c\u6b63\u5728\u5bfb\u627e\u4e0a\u4e00\u5c42\u7684\u5feb\u7167\u3002");
        ...
        {
            SetStatusText("\u5df2\u786e\u8ba4\u56de\u9000\u5230\u4e0a\u4e00\u6b65\uff0c\u6b63\u5728\u5bfb\u627e\u4e0a\u4e00\u56de\u5408\u7684\u5feb\u7167\u3002");
    ...
    private bool TryHandleMultiplayerApproval(string actionLabel)
    {
        ...
        if (actionKind == RaiseHandActionKind.PreviousStep)
        {
            SetStatusText("\u795e\u79d8\u529b\u91cf\u6d88\u6563\u4e86\u3002");
            ShowInfoPopup(
                RaiseHandPopupText.MultiplayerPreviousStepDisabledTitleText,
                RaiseHandPopupText.MultiplayerPreviousStepDisabledBodyText);
            return true;
        }

        if (actionKind == RaiseHandActionKind.SoftRestart && RunManager.Instance.DebugOnlyGetState()?.CurrentRoom is MapRoom)
        {
    ...
    private void ConfigureConfirmPopup(NDisconnectConfirmPopup popup, string actionLabel)
    {
        ...
        verticalPopup.SetText(ConfirmTitleText, actionLabel == PreviousStepLabel ? PreviousStepConfirmBodyText : ConfirmBodyText);
    ...
    private async System.Threading.Tasks.Task ReloadPreviousCombatStepFromBackup()
    {
        try
        {
            ...
            var isCombatRoom = combatRoom != null;
            var isSoftRestartRoom = currentRoom is MerchantRoom or EventRoom or RestSiteRoom or TreasureRoom;
            if (!HasPreviousStepSnapshotForCurrentMode() && IsAtInitialStepBoundary())
            {
                SetStatusText("\u4f60\u5df2\u7ecf\u6ca1\u6709\u9000\u8def\u4e86\u9a9a\u5e74\u3002");
                ShowInfoPopup(RaiseHandPopupText.NoWayBackTitleText, RaiseHandPopupText.NoWayBackBodyText);
                return;
            }

            if (!isCombatRoom && !isSoftRestartRoom)
            {
            ...
            if (IsBossOrEliteCombat(runState, combatRoom))
            {
                SetStatusText("\u738b\u5bf9\u738b\u3002");
                ShowInfoPopup(RaiseHandPopupText.BossEliteNoStepBackTitleText, RaiseHandPopupText.BossEliteNoStepBackBodyText);
                return;
            }

            if (isCombatRoom && IsAtFirstCombatStepBoundary(combatRoom))
            {
                SetStatusText("\u4f60\u5df2\u65e0\u8def\u53ef\u9000\u3002");
    ...
    private async System.Threading.Tasks.Task LoadRunIntoCurrentRoom(SerializableRun serializableRun, SerializableRoom roomSnapshot, string? combatStateJson)
    {
        ...
        if (restoredRoom is CombatRoom && RunManager.Instance.CombatReplayWriter.IsEnabled)
        ...
        try
        {
            if (restoredRoom is CombatRoom)
            {
                ...
                SingleplayerPreviousStepBannerSuppressionService.SuppressNextCombatStartPresentation();
            }

            await RunManager.Instance.EnterRoom(restoredRoom);
            await SingleplayerCombatStateSnapshotService.TryApplyCombatStateJsonAsync(combatStateJson, runState, "singleplayer previous step");
        }
        finally
        {
            SingleplayerPreviousStepRestoreStateService.EndRestore();
            SingleplayerPreviousStepBannerSuppressionService.Clear();
            await NGame.Instance.Transition.FadeIn(0.8f, "res://materials/transitions/fade_transition_mat.tres", null);
        }
    }
    ...
    private static bool IsAtFirstCombatStepBoundary(CombatRoom combatRoom)
    {
        return combatRoom.CombatState.RoundNumber <= 1;
    }

    private static bool IsAtInitialStepBoundary()
    {
        var runState = RunManager.Instance.DebugOnlyGetState();
        return runState != null
               && runState.TotalFloor <= 1
               && !HasPreviousFloorSnapshotForCurrentMode();
    }

    private static bool IsBossOrEliteCombat(RunState? runState, CombatRoom combatRoom)
    {
        if (runState == null || combatRoom.IsPreFinished)
            return false;
        ...
        return mapPointType is MapPointType.Boss or MapPointType.Elite;
    }
    ...
    private static bool HasPreviousStepSnapshotForCurrentMode()
    {
        return NonStandardSingleplayerRunIdentity.ShouldUseIsolatedSnapshots(RunManager.Instance)
