...
public static class PreviousFloorSnapshotService
{
    private const string SnapshotDirectoryName = "handsup_previous_floor_history";
    ...
    public static void CaptureSnapshotFromCurrentState(RunManager? runManager)
    {
        ...
        if (!TryBuildRunKey(runManager, out var runKey))
            return;
        ...
        MainFile.Logger.Info($"Previous-floor snapshot written to {snapshotPath}");
    }

    public static bool HasSnapshot(RunManager? runManager)
    {
        return TryBuildRunKey(runManager, out var runKey) && GetSnapshotFiles(runKey).Count > 0;
    }
    ...
    public static void ClearSnapshots(string reason)
    {
        ...
        MainFile.Logger.Info($"Cleared previous-floor snapshots: {reason}");
    }

    public static SerializableRun RestoreLatestSnapshot(RunManager? runManager)
    {
        if (!TryBuildRunKey(runManager, out var runKey))
            throw new FileNotFoundException("Previous floor snapshot scope was not found.");

        var latestSnapshot = GetLatestSnapshotFile(runKey)
                             ?? throw new FileNotFoundException("Previous floor snapshot was not found.");
        var snapshot = ReadSnapshot(latestSnapshot.FullName);

        File.Delete(latestSnapshot.FullName);

        MainFile.Logger.Info($"Previous-floor snapshot restored from {latestSnapshot.FullName}");
        return snapshot;
    }

    public static SerializableRun PeekLatestSnapshot(RunManager? runManager)
    {
        if (!TryBuildRunKey(runManager, out var runKey))
            throw new FileNotFoundException("Previous floor snapshot scope was not found.");
    ...
    public static void DiscardLatestSnapshot(RunManager? runManager, string reason)
    {
        if (!TryBuildRunKey(runManager, out var runKey))
            return;
        ...
        MainFile.Logger.Info($"Discarded previous-floor snapshot {latestSnapshot.Value.FullName}: {reason}");
    }
    ...
    private static bool TryBuildRunKey(RunManager? runManager, out string runKey)
    {
        runKey = string.Empty;
        var snapshot = runManager?.ToSave(null);
        if (snapshot == null)
        ...
        runKey = $"{snapshot.StartTime:D16}_{SanitizeFilePart(characterId)}_{SanitizeFilePart(seed)}";
        return true;
    }
    ...
    private static string SanitizeFilePart(string value)
    {
        ...
        return new string(value.Select(ch => invalidChars.Contains(ch) ? '_' : ch).ToArray()).Replace(':', '_');
    }
    ...
    private static SerializableRun ReadSnapshot(string snapshotPath)
    {
        ...
        if (!readResult.Success || readResult.SaveData == null)
            throw new InvalidDataException($"Failed to parse previous floor snapshot: {snapshotPath}");
